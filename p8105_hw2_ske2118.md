...
These data … The number of observations in this dataset are 1033 and key
variables are wheel, dumpster, month, year, date, and
volume_cubic_yards.
...
```
    ...
    ## Warning in left_join(results_bakersbakes_merge, viewers_df, by = c("series")): Detected an unexpected many-to-many relationship between `x` and `y`.
